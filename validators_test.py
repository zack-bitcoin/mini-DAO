...
def add_validator(location, addr, balance, accs, c, nonce):
    f=mk_mk_sig(hash_list([addr, location, balance, nonce, 1337]))
    c.update(addr, location, balance, reduce(add, map(f, accs)))
def test(c):
    accs=map(mk_acc, [1,2,3])
    balance = 10000000
    add_validator(1, accs[1]["addr"], balance, accs[0:1], c, 0)
    add_validator(2, accs[2]["addr"], balance, accs[0:2], c, 1)
